...
import { CardField, CardFieldInput, useStripe, StripeContainer, Token, } from '@stripe/stripe-react-native';
...
import { ADD_CARDS, requestPostApi, GET_CARDS, requestGetApi, getApiWithToken, BUY_PLAN } from '../Global/Service';
...
const Payment = (props, route) => {
    ...
    const handlePayClick = async () => {
        ...
        console.log('responseJson.headers.success for add card---->>>', responseJson);
    ...
    const placeOrder = async () => {
        ...
        if (checkitem == '') {
            ...
            const { responseJson, err } = await requestPostApi(BUY_PLAN, data, 'POST', userToken)

            console.log("requestPostApiErrREsp", responseJson, err);
            setLoading(false)
            console.log('the res shop_eat_cart_place_order==>>', responseJson.status)
    ...
    return (
        <SafeAreaView scrollEnabled={scrollEnabled} style={{ height: '100%', backgroundColor: '#F8F8F8' }}>
                    ...
                    >
                        ...
                        <View style={{ width: '92%', alignSelf: 'center' }}>
                            ...
                            <View style={{ width: '100%', flexDirection: 'row', justifyContent: 'space-between', alignSelf: 'center', marginVertical: 15 }}>
                                ...
                                <Text style={{ color: Color.LIGHT_BLACK, fontWeight: '500', fontSize: 12 }}>CHOOSE CARD</Text>
                                <TouchableOpacity>
                                    <TouchableOpacity onPress={() => {
                                        setaddpayment(true)
                                    }}>
                                        <Text style={{ color: Color.PRIMARY, fontWeight: '500', textDecorationLine: 'underline', fontSize: 13 }} onPress={() => {
                                            setaddpayment(true)
                                        }}>Add New Card</Text>
                                    </TouchableOpacity>
                                </TouchableOpacity>
                            ...
                            {addpayment ?
                                <KeyboardAvoidingView
                                    ...
                                    <View style={{ width: '100%', marginTop: -10 }}>
                                        <MyButton title="Add" height={53} width={'100%'} borderRadius={5} alignSelf="center"
                                            press={() => { handlePayClick() }}
                                            marginHorizontal={20} fontSize={14}
                                            titlecolor={Color.WHITE} backgroundColor={Color.PRIMARY} />
                                        {/* <View>
                                            <Text>kkkkkkk</Text>
                                        </View> */}
                                    </View>
                            ...
                            }
                            {
                                allCardList.map((item, index) => {
                                    ...
                                    return (
                                        ...
                                        <>
                                            {checkitem != item ?
                                                <TouchableOpacity style={{ width: '100%', borderColor: 'red', borderWidth: 0.02, flexDirection: 'row', alignItems: 'center', paddingVertical: 17, paddingHorizontal: 17, borderRadius: 7, backgroundColor: '#fff', marginTop: 15 }}
                                                    onPress={() => {
                                                        setcheckitem(item)
                                                        console.log('my checked item for the card--->>', item);
                                                    }}>
                                                    ...
                                                    <View style={{ marginLeft: 15 }}>
                                                        <Text style={{ color: Color.TEXT_COLOR, fontWeight: '500', fontSize: 13 }}>**** **** **** {item.last4}</Text>
                                                        <Text style={{ color: 'red', fontWeight: '400', fontSize: 11, top: 2 }}>{item.id == '' ? '' : 'Expires :'} {item.exp_month}{item.id == '' ? '' : '/'}{item.exp_year}</Text>
                                                    </View>
                                                </TouchableOpacity>
                                                :
                                                <></>
                            ...
                            {checkitem.length != 0 ?
                                ...
                                <><Text style={{ color: 'red', fontWeight: '300', fontSize: 12, marginVertical: 20 }}>CURRENT METHOD</Text>
                                    <View style={{ width: '100%', borderColor: '#FFC40C', borderWidth: 0.2, flexDirection: 'row', alignItems: 'center', paddingVertical: 17, paddingHorizontal: 17, borderRadius: 7, backgroundColor: '#fff' }}>
                                        ...
                                        <View style={{ marginLeft: 15 }}>
                                            <Text style={{ color: Color.TEXT_COLOR, fontWeight: '500', fontSize: 13 }}>**** **** **** {checkitem.last4}</Text>
                                            <Text style={{ color: 'red', fontWeight: '400', fontSize: 10, top: 2 }}>{checkitem.id == '' ? '' : 'Expires :'} {checkitem.exp_month}{checkitem.id == '' ? '' : '/'}{checkitem.exp_year}
                                            </Text>
                                        </View>
                                        <View style={{ width: 24, height: 24, borderRadius: 5, position: 'absolute', right: 20, justifyContent: 'center', }}>
                                            <TickDark ></TickDark>
                            ...
                            <View style={{ width: '100%', marginTop: 30 }}>
                                <MyButtons title="Proceed to Payment" height={53} width={'100%'} borderRadius={5} alignSelf="center" press={() => {
                                    placeOrder()
                                    // props.navigation.goBack()


                                }} marginHorizontal={20} fontSize={14}
                                    titlecolor={'white'} backgroundColor={Color.PRIMARY} />
                            </View>
                        </View>
            ...
            >
                <View style={{ height: 150, backgroundColor: 'green', borderTopLeftRadius: 30, borderTopRightRadius: 30, padding: 20, margin: 0, bottom: 0 }}>
                    <View style={styles.mainView}>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', width: '100%' }}>
                            <TouchableOpacity style={{ width: 150, height: 150 }} onPress={onGallery}>
                                ...
                                <Text style={{ textAlign: 'center', color: Color.TEXT_COLOR }}>Open Library</Text>
                            </TouchableOpacity>
                            <TouchableOpacity style={{ width: 150, height: 150 }} onPress={onCamera}>
                                ...
                                <Text style={{ textAlign: 'center', color: Color.TEXT_COLOR }}>Open Camera</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                </View>
