    ...
    const RenderItemLead = ({ item }) => {
        ...
        return (
            <TouchableOpacity style={styles.moduleView} onPress={() => {
                ...
                {item?.type === 'pdf' ? <Pdf height={40} width={40} style={{ justifyContent: 'center', marginTop: 12 }}></Pdf> : item?.type === 'worksheet' ? <Worksheet height={40} width={40} style={{ justifyContent: 'center', marginTop: 12 }}></Worksheet> : item?.type === 'quiz' ? <Quiz height={42} width={42} style={{ justifyContent: 'center', marginTop: 8 }}></Quiz> : item?.type === 'video' ? <Video height={40} width={40} style={{ justifyContent: 'center', marginTop: 12 }}></Video> : < Module ></Module>}
                <View>
                    <MyText
                        text={`${item?.title === null ? '' : item.title.length > 25 ? `${item?.title.substring(0, 25)}.....` : `${item?.title.substring(0, 25)}`}`}
                        fontFamily="Roboto"
                        fontWeight='bold'
                        fontSize={15}
                    ...
                    />
                    <MyText
                        text={`${item?.description === null ? '' : item.description.length > 30 ? `${item?.description.substring(0, 30)}.....` : `${item?.description.substring(0, 20)}`}`}
                        fontFamily="Roboto"
                        fontWeight='500'
                        fontSize={14}
    ...
    return (
        <SafeAreaView style={{ flex: 1 }}>
            ...
            <View style={styles.container}>
                <MyHeader Title="List of Modules" isBackButton
                    scrolling={scrolling}
                    scrollY={scrollY}
