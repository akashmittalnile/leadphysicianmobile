...
import { GET_PROFILE, postApiWithToken, LOGOUT, getApiWithToken, GET_GROUPS, READ_CHATADMIN } from '../../Global/Service';
...
const Chat = ({ navigation }) => {
    ...
    const RenderChat = ({ item }) => {
        console.log('item ');
        return (
            <TouchableOpacity style={styles.chatView} onPress={() => { navigation.navigate('ChatDetail', { id: item.id }) }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 10, paddingHorizontal: 12 }}>
                    <View style={{ flexDirection: 'row' }}>
                        <GroupChat></GroupChat>
                        <View>
                            <MyText text={item.name} fontWeight='400' fontSize={14} textColor={Color.LIGHT_BLACK} fontFamily='Roboto' style={{ textAlign: 'center', marginHorizontal: 16 }} />
                ...
                <View style={styles.bottomChat}>
                    <View style={styles.rowIndivi}>
                        ...
                        <MyText text={`${item?.users?.length} members`} fontWeight='bold' fontSize={13} textColor={Color.BLACK} fontFamily='Roboto' style={{ marginHorizontal: 12, marginVertical: 5 }} />
                    </View>
