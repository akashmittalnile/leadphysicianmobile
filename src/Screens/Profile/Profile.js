        ...
        return (
            <TouchableOpacity style={styles.scduleView} onPress={() => {
                item.id === '1' ? navigation.navigate('ChangePassword') :
                    item.id === '2' ? navigation.navigate('TermsCondition', { params: 'Privacy' })
                        :
                        item.id === '3' ? navigation.navigate('TermsCondition', { params: 'Policy' })
                            : item.id === '4' ? logout() : item.id === '0' ? navigation.navigate('Certificate',) : null
            }}>
                            ...
                            <View style={{ position: 'absolute', backgroundColor: '#F7FAEB', height: 48, width: dimensions.SCREEN_WIDTH, bottom: 0, alignSelf: 'center' }}>
                                <View style={{ flexDirection: 'row', width: dimensions.SCREEN_WIDTH * 0.80, alignSelf: 'center', justifyContent: 'space-between' }}>
                                    ...
                                    <View style={{ flexDirection: 'row', justifyContent: 'center', marginVertical: 12 }}>
                                        ...
                                        <MyText text=
                                            {`${profile?.email === undefined ? '' : profile?.email?.length > 19 ? `${profile?.email.substring(0, 19)}.....` : `${profile?.email.substring(0, 19)}`}`
                                            } fontWeight='400' fontSize={14} textColor={'#66757F'} fontFamily='Roboto' style={{ marginHorizontal: 5, marginVertical: 3 }} />
                                    </View>
                                </View>
                            </View>
                        </View>
                            ...
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', width: dimensions.SCREEN_WIDTH * 0.90, alignItems: 'center' }}>
                                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                    ...
                                    <View>
                                        ...
                                        <MyText text={currentPlan.name} fontWeight='400' fontSize={16} textColor={Color.WHITE} fontFamily='Roboto' style={{}} />
                                        <MyText text={`$ ${currentPlan?.price ? currentPlan?.price : currentPlan?.monthly_price}`} fontWeight='500' fontSize={24} textColor={Color.WHITE} fontFamily='Roboto' style={{}} />
                                    </View>
                                </View>
                                <TouchableOpacity style={{ width: 103, height: 44, borderRadius: 5, backgroundColor: 'white', justifyContent: 'center' }} onPress={() => { navigation.navigate('Subscription') }}>
